...
test('no hint means no overlay and no script — a normal browser pays nothing', async () => {
  ...
  assert.ok(!html.includes('data-hint-close'), 'no fallback script wired');
...
for (const [name, props, url] of [
  ...
  test(`${name} card: the one selector matches the app CTA it must hook`, async () => {
    ...
    assert.ok(html.includes('data-hint'), 'overlay renders');
    assert.ok(html.includes('data-hint-close'), 'the dismiss control exists');
...
test('the label is untouched — the button still says what the rider wanted', async () => {
  ...
  assert.ok(html.includes('https://apps.apple.com/app/id6765577701'), 'the store link survives inside the overlay');
});

test('the overlay ships hidden, so it never covers the card until the timer fires', async () => {
  const html = await render({ ...profile, appCTA: { label: 'Open in the app', url: 'dirtbikex://s/u/calvin' }, browserHint: hint });
...
test('copy is escaped, not interpolated raw', async () => {
  const html = await render({
    ...
    browserHint: { ...hint, title: '<script>alert(1)</script>' },
  });
